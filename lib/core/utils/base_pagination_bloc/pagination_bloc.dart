...
abstract class PaginationBloc<Event, Entity>
  ...
  PaginationBloc({required int initialPage})
      : super(PaginationState<List<Entity>>(maxResultCount: initialPage));

  FutureOr<void> fetchDataFirstTime(
      Event event, Emitter<PaginationState<List<Entity>>> emit);
  ...
  void _emitFailure(
      Failure failure, Emitter<PaginationState<List<Entity>>> emit) {
  ...
  PaginationState<List<Entity>> _appendPage(
      List<Entity> newItems, bool hasReachedMax) {
    final data = state.data ?? [];
    ...
    return state.success(updatedData, page, hasReachedMax);
  }
